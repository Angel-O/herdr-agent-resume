...
NAME="herdr-agent-resume"
REPO="Angel-O/herdr-agent-resume"

script_dir=$(CDPATH= cd "$(dirname "$0")" && pwd)
root=$(CDPATH= cd "$script_dir/.." && pwd)
manifest="$root/herdr-plugin.toml"
destination=${HAR_INSTALL_PATH:-"$root/target/release/$NAME"}
base_url=${HAR_RELEASE_BASE_URL:-"https://github.com/$REPO/releases/download"}
